...
describe('Sudoku', function() {
  ...
  describe('unsolve', function() {
    it("returns a sudoku puzzle that solves to the sudoku's current grid", function() {
      ...
      let sudoku = new Sudoku(board);
      let puzzle = sudoku.unsolve("easy");
      console.log(puzzle);
      let sudoku2 = new Sudoku(puzzle);
      expect(sudoku2.solve()).toEqual([sudoku.board]);
    });

    it("allows different difficulty levels", function() {
      let board = [
        [1,7,4,2,8,5,3,6,9],
        [8,3,5,6,9,4,2,1,7],
        [6,9,2,1,7,3,8,4,5],
      ...
      let sudoku = new Sudoku(board);
      let puzzle = sudoku.unsolve("hard");
      console.log("hard");
      console.log(puzzle);
      let puzzle2 = sudoku.unsolve("medium");
      console.log("medium");
      console.log(puzzle2);
      let sudoku2 = new Sudoku(puzzle);
      let sudoku3 = new Sudoku(puzzle2);
      expect(sudoku2.solve()).toEqual([sudoku.board]);
      expect(sudoku3.solve()).toEqual([sudoku.board]);
    });
  });
